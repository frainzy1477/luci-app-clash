...
sys.call("awk -F ',' '{print $4}' /usr/share/clash/provider/ruleproviders/rule_provider.list > /tmp/rule_providers 2>/dev/null")
...
e[t].name=string.sub(luci.sys.exec(string.format("grep -F ',%s' /usr/share/clash/provider/ruleproviders/rule_provider.list |awk -F ',' '{print $1}' 2>/dev/null",o)),1,-2)

e[t].behaviour=string.sub(luci.sys.exec(string.format("grep -F ',%s' /usr/share/clash/provider/ruleproviders/rule_provider.list |awk -F ',' '{print $2}' 2>/dev/null",o)),1,-2)

e[t].filename=string.sub(luci.sys.exec(string.format("grep -F '%s,' /usr/share/clash/provider/ruleproviders/rule_provider.list |awk -F ',' '{print $4}' 2>/dev/null",o)),1,-2)
