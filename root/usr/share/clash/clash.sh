...
wget-ssl --no-check-certificate --user-agent="User-Agent: Luci-app-clash/OpenWRT" $subscribe_url -O 2>&1 >1 $CONFIG_YAML
elif [ $subtype == "v2rayn2clash" ];then
if [ $cusrule == 1 ];then
wget-ssl --no-check-certificate --user-agent="User-Agent: Luci-app-clash/OpenWRT" $urlv2ray.$subscribe_url -O 2>&1 >1 $CONFIG_YAML_TEMP
if [ -f $CONFIG_YAML_TEMP ];then
sed -i '/Rule:/,$d' $CONFIG_YAML_TEMP 
cat $CONFIG_YAML_TEMP $CONFIG_YAML_RULE > $CONFIG_YAML 
fi
else
wget-ssl --no-check-certificate --user-agent="User-Agent: Luci-app-clash/OpenWRT" $urlv2ray.$subscribe_url -O 2>&1 >1 $CONFIG_YAML
fi
elif [ $subtype == "surge2clash" ];then
wget-ssl --no-check-certificate --user-agent="User-Agent: Luci-app-clash/OpenWRT" $urlsurge.$subscribe_url -O 2>&1 >1 $CONFIG_YAML
fi
