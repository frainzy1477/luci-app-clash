...
new_clashtun_core_version=`curl -sL ${clash_curl_proxy} "https://github.com/comzyh/clash/tags"| grep "a href=\"/comzyh/clash/releases" | head -n 1| awk -F "/tag/" '{print $2}'| sed 's/\">//'`
if [ "$?" -eq "0" ]; then
